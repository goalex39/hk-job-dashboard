...
def categorize_news(item):
    """Auto-assign category based on headline keywords."""
...
def assign_sentiment(item):
    """Auto-assign sentiment based on keywords."""
    h = item["headline"].lower()
...
def fetch_all_news(existing_news):
    ...
    # Categorize and assign sentiment
    new_items = [assign_sentiment(categorize_news(item)) for item in new_items]
