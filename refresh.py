...
# ===========================================================
# SECTION 3: JOB LISTINGS FETCHERS — HTTP scraping + verified URLs
# ===========================================================

# Search result page URLs (always valid, users click through to individual jobs)
JOBSDB_SEARCH = "https://hk.jobsdb.com/data-governance-jobs/in-Hong-Kong"
JOBSDB_SEARCH_AI = "https://hk.jobsdb.com/ai-governance-jobs/in-Hong-Kong"
LINKEDIN_SEARCH = "https://www.linkedin.com/jobs/search/?keywords=data+governance&location=Hong+Kong"
LINKEDIN_SEARCH_AI = "https://www.linkedin.com/jobs/search/?keywords=AI+governance&location=Hong+Kong"
EFIN_SEARCH = "https://www.efinancialcareers.com/jobs?keywords=data+governance&location=Hong+Kong"
INDEED_SEARCH = "https://hk.indeed.com/jobs?q=data+governance&l=Hong+Kong"


def fetch_jobsdb_real():
    """Scrape real job listings from JobsDB search results page."""
    print("  JobsDB...")
    jobs = []
    for search_url in [JOBSDB_SEARCH, JOBSDB_SEARCH_AI]:
        try:
            resp = requests.get(search_url, headers=HEADERS, timeout=20)
            if resp.status_code != 200:
                print(f"    [WARN] JobsDB returned {resp.status_code}")
                continue
            soup = BeautifulSoup(resp.text, "html.parser")

            # JobsDB uses data attributes and article elements
            articles = soup.select('article[data-card-type="JobCard"]')
            if not articles:
                # Fallback: look for job card links
                articles = soup.select('div[data-search-sol-meta]')
            if not articles:
                # Try finding any link with /job/ pattern
                ...
                for link in links[:15]:
                    ...
                    if not title or len(title) < 5 or "/job/" not in href:
                        continue
                    full_url = href if href.startswith("http") else "https://hk.jobsdb.com" + href
                    ...
                    jobs.append({
                        ...
                        "desc": title,
                        "source": "JobsDB",
                        "url": full_url.split("?")[0],  # clean URL
                        "posted": datetime.now(HKT).strftime("%d %b %Y")
                    })

            for article in articles[:15]:
                # Try to extract structured data
                title_el = article.select_one('a[data-automation="jobTitle"], h3 a, a[href*="/job/"]')
                company_el = article.select_one('a[data-automation="jobCompany"], span[data-automation="jobCompany"]')
                loc_el = article.select_one('span[data-automation="jobLocation"], a[data-automation="jobLocation"]')
                salary_el = article.select_one('span[data-automation="jobSalary"]')
                date_el = article.select_one('span[data-automation="jobListingDate"], time')
                ...
                jobs.append({
                    ...
                    "posted": date_el.get_text(strip=True) if date_el else datetime.now(HKT).strftime("%d %b %Y")
                })
        except Exception as e:
            print(f"    [WARN] JobsDB scrape failed: {e}")
    ...
    for j in jobs:
        if j["url"] not in seen:
            seen.add(j["url"])
            unique.append(j)
    print(f"    Found {len(unique)} listings")
    return unique[:15]


def fetch_indeed_real():
    """Scrape real job listings from Indeed HK search results."""
    print("  Indeed...")
    jobs = []
    queries = ["data+governance", "AI+governance", "data+protection+officer"]
    for q in queries:
        try:
            url = f"https://hk.indeed.com/jobs?q={q}&l=Hong+Kong"
            resp = requests.get(url, headers=HEADERS, timeout=20)
            if resp.status_code != 200:
                continue
            soup = BeautifulSoup(resp.text, "html.parser")
            # Indeed job cards
            cards = soup.select('.job_seen_beacon, .resultContent, .jobsearch-ResultsList > li')
            for card in cards[:8]:
                ...
                if not href:
                    continue
                # Indeed uses jk= parameter for job key
                full_url = href if href.startswith("http") else "https://hk.indeed.com" + href
                # Simplify to viewjob URL if we can extract jk
                jk_match = re.search(r'jk=([a-f0-9]+)', full_url)
                if jk_match:
                    full_url = f"https://hk.indeed.com/viewjob?jk={jk_match.group(1)}"
            ...
            print(f"    [WARN] Indeed scrape failed for '{q}': {e}")
...
def fetch_efinancialcareers_real():
    """Scrape real job listings from eFinancialCareers."""
    print("  eFinancialCareers...")
    jobs = []
    try:
        resp = requests.get(EFIN_SEARCH, headers=HEADERS, timeout=20)
        if resp.status_code == 200:
            soup = BeautifulSoup(resp.text, "html.parser")
            # eFC uses specific card structures
            cards = soup.select('div[class*="JobCard"], div[class*="job-card"], article, .job-result')
            links = soup.select('a[href*="/jobs-"][href*=".id"]')
            for link in links[:10]:
                href = link.get("href", "")
                title = link.get_text(strip=True)
                if not title or len(title) < 5:
                    continue
                full_url = href if href.startswith("http") else "https://www.efinancialcareers.com" + href
                jobs.append({
                    "title": title,
                    "company": "See listing",
                    "location": "Hong Kong",
        ...
        print(f"    [WARN] eFinancialCareers scrape failed: {e}")
...
def create_search_link_jobs():
    """
    Create reliable "search all" link cards for each job board.
    These always work because they link to search result pages, not individual jobs.
    """
...
def fetch_all_jobs():
    """
    Fetch jobs from all sources. Individual scraped listings come first,
    followed by search link cards as guaranteed-valid fallbacks.
    THIS is what keeps the Job Openings tab current.
    """
    print("\n[JOBS] Fetching real job listings...")
    all_jobs = []

    # Try to scrape real individual listings
    all_jobs.extend(fetch_jobsdb_real())
    all_jobs.extend(fetch_indeed_real())
    all_jobs.extend(fetch_efinancialcareers_real())
    # LinkedIn blocks unauthenticated scraping, skip individual listings

    # Always append search link cards at the end as fallback/browse links
    all_jobs.extend(create_search_link_jobs())

    # Verify all URLs are valid by checking they start with http and contain the expected domain
    verified = []
    for j in all_jobs:
        url = j.get("url", "")
        if not url.startswith("http"):
            continue
        # Verify domain matches source
        domain_map = {
            "JobsDB": "jobsdb.com",
        ...
        expected_domain = domain_map.get(j["source"], "")
        if expected_domain and expected_domain not in url:
            print(f"    [DROP] URL domain mismatch: {url} (expected {expected_domain})")
            continue
        verified.append(j)

    print(f"  Total verified listings: {len(verified)}")
    return verified
