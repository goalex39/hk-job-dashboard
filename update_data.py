...
def extract_json(text):
    text = re.sub(r"^```json\s*", "", text.strip(), flags=re.MULTILINE)  # FIX 6
    text = re.sub(r"^```\s*",     "", text.strip(), flags=re.MULTILINE)  # FIX 6
    text = re.sub(r"```$",        "", text.strip(), flags=re.MULTILINE)  # FIX 6
...
def ask_perplexity(user_msg, model="sonar"):
    ...
    return resp.choices.message.content  # FIX 2


# =============================================================================
# LIVE KPI 1 — Hang Seng Index
# =============================================================================
...
if yahoo:
    try:
        meta  = yahoo["chart"]["result"]["meta"]  # note: result is a list
...
# =============================================================================
# LIVE KPI 2 — US Fed Funds Rate (FRED)
# =============================================================================
...
if FRED_API_KEY:
    ...
    if fred and fred.get("observations"):
        obs        = fred["observations"]       # obs is a list
        rate       = float(obs["value"])      # FIX 3
        fed_val    = "{:.2f}%".format(rate)
        fed_period = obs["date"][:7] + " . Federal Reserve"
        if len(obs) > 1:
            diff = rate - float(obs["value"]) # FIX 3[1]
            if abs(diff) > 0.001:
...
# =============================================================================
# MANUAL KPIs — update values below each month
# =============================================================================
print("\n[3/4] Building KPI block...")
...
NEWS_PROMPT = (
    ...
    "Include at least 15 items covering: HK macro data, US-China tariffs, AI/data regulation "
...
# =============================================================================
# JOBS via Perplexity sonar
# =============================================================================
print("\n[4b/4] Job listings via Perplexity sonar...")

JOBS_PROMPT = (
    "Search for CURRENT job openings posted in the last 30 days in Hong Kong for:\n"
    "Data Governance, AI Governance, Data Privacy, DPO, Chief Data Officer,\n"
    "Head of Data, Data Quality, DPIA Analyst, Privacy Analyst,\n"
    ...
    "  desc     - string (2 sentences: role summary + key requirements)\n\n"
    "Include at least 20 listings. Mix seniority: analyst, manager, AVP, VP, director.\n"
    "Prioritise roles valuing: CIPP/E, CIPM, CDMP, CRISC, DAMA, FIP, banking experience."
)

jobs_data = []  # FIX 4 — consistent lowercase variable name throughout
try:
...
n_jobs = len(jobs_data)  # FIX 4

output_lines = [
    "// Auto-generated by update_data.py — DO NOT EDIT MANUALLY",
    "// Last updated: " + NOW,
    "// KPI_DATA: " + str(n_kpi) + " | NEWS_DATA: " + str(n_news) + " | JOBS_DATA: " + str(n_jobs),
    "",
